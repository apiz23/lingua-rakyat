...
// Draw a single oval with soft edges and calculate its linear color
bool drawOval(vec2 polarUv, vec2 polarCenter, float a, float b, bool reverseGradient, float softness, out vec4 color) {
    ...
    if (edge > 0.0) {
        float linear = reverseGradient ? (1.0 - (p.x / a + 1.0) / 2.0) : ((p.x / a + 1.0) / 2.0);
        // Flatten linear toward middle value for more uniform appearance
        linear = mix(0.5, linear, 0.1);
        color = vec4(vec3(linear), 0.85 * edge);
        return true;
    }
    return false;
}
...
void main() {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        bool reverseGradient = (i % 2 == 1); // Reverse linear for every second oval
