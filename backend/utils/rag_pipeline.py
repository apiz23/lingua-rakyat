...
def _qa_prompt(context: str, question: str, lang: str) -> str:
    prompts = {
        "en": (
            ...
            "Use simple everyday language, like a friendly officer explaining to a citizen.\n"
            "Reply conversationally in 2-5 sentences by default.\n"
            "Include relevant supporting details from the context (amounts, eligibility, related steps) so the citizen gets the full picture, not just the bare fact.\n"
            "Use a bullet list ONLY when listing steps, requirements, or several distinct items.\n"
            "Use **bold** for key amounts, dates, and document names.\n"
        ...
        "ms": (
            ...
            "Gunakan bahasa mudah, seperti pegawai mesra menerangkan kepada rakyat.\n"
            "Jawab secara perbualan dalam 2-5 ayat pendek secara lalai.\n"
            "Sertakan butiran sokongan yang relevan daripada konteks (jumlah, kelayakan, langkah berkaitan) supaya rakyat dapat gambaran penuh, bukan sekadar fakta ringkas.\n"
            "Guna senarai mata peluru HANYA apabila menyenaraikan langkah, syarat, atau beberapa perkara berasingan.\n"
            "Guna **tebal** untuk jumlah, tarikh, dan nama dokumen penting.\n"
        ...
        "zh-cn": (
            "你是一位政府服务助手。\n"
            "只能根据上下文回答。\n"
            "使用简单易懂的语言，像亲切的公务员向市民解释一样。\n"
            "默认用2到5句简短的话自然回答。\n"
            "请包含上下文中相关的支持细节（金额、资格、相关步骤），让市民了解全貌，而不只是一个简单事实。\n"
            "只有在列出步骤、条件或多个不同项目时才使用要点列表。\n"
            "重要金额、日期和文件名称请用**加粗**标出。\n"
...
def _cautious_qa_prompt(context: str, question: str, lang: str) -> str:
    prompts = {
        "en": (
            ...
            "Reply conversationally in 2-5 short sentences by default.\n"
            "Include relevant supporting details from the context (amounts, eligibility, related steps) so the citizen gets the full picture, not just the bare fact.\n"
            "Use a bullet list ONLY when listing steps, requirements, or several distinct items.\n"
            "End with: Source: Based on official documents provided.\n\n"
            "Context:\n{context}\n\n"
...
def _insufficient_evidence_answer(lang: str) -> str:
    answers = {
        "en": (
            "Sorry — I can't answer this confidently from the uploaded documents. "
            "The closest passages I found don't directly address your question.\n\n"
            "Try asking with more specific terms, or upload a document that contains this information. "
            "You can also open the source excerpt below to check the closest match.\n\n"
            "Source: Based on official documents provided."
        ),
        "ms": (
            "Maaf — saya tidak dapat menjawab dengan yakin berdasarkan dokumen yang dimuat naik. "
            "Petikan terdekat yang ditemui tidak menjawab soalan anda secara langsung.\n\n"
